...
public class StreamingConnector extends Connector {
	...
	private StringBuilder streamReaderStringBuilder = new StringBuilder();
		...
		try {
			while (streamConnected
				...
				if ("\n".equals(newline) || "".equals(newline)) {
					messageString = streamReaderStringBuilder.toString();
					streamReaderStringBuilder.setLength(0);
					readDone = true;
				} else {
					streamReaderStringBuilder.append(newline);
				}
				newline = null;
			}
			if (!sockOK) {
